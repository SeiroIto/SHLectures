...
![PowerShell 7 のインストール手順ページ。MSI パッケージのリンクから入れます](setup_shots/pwsh_install.png){width=85%}
...
![CRAN の Windows 版ページ。上端のリンク1つだけ押します](setup_shots/cran_windows.png){width=85%}
...
![Quarto のダウンロードページ。青いボタン、または表の Windows 行の .msi](setup_shots/quarto_download.png){width=85%}
...
![サクラエディタ公式サイト。最新版ダウンロードから入ります](setup_shots/sakura_top.png){width=85%}
...
![Notepad++ のダウンロードページ。最新版を選び、portable zip を落とします](setup_shots/nppp_downloads.png){width=85%}
...
![R3.kwd の配布ページ](setup_shots/sakura_r_kwd.png){width=85%}

**Markdown**

1. <http://sakura.qp.land.to/?Customize%2F%C5%EA%B9%C6%2F77> を開く
2. `markdown2.rkw` と `Markdown2.col` を保存する
3. 設定 → タイプ別設定 → 「正規表現キーワード」タブ →
   「正規表現キーワードを使用する」に印 → 「インポート」で `markdown2.rkw`
4. 同じ画面の「カラー」タブ → 「インポート」で `Markdown2.col`

![サクラエディタ用 Markdown 設定の配布ページ](setup_shots/sakura_markdown.png){width=85%}
...
![Notepad++ の定義ファイル一覧。R language の行](setup_shots/nppp_udl_list.png){width=85%}
...
![clink の Releases ページ。Assets の中の setup.exe を落とします](setup_shots/clink_releases.png){width=85%}
...
[作例1を開く](examples/example_basic.html) ・
[qmd を見る](examples/example_basic.qmd)

```{r}
#| results: asis
cat("`````markdown", readLines("examples/example_basic.qmd"), "`````", sep = "\n")
```
...
[作例2を開く](examples/example_tufte.html) ・
[qmd を見る](examples/example_tufte.qmd)

```{r}
#| results: asis
cat("`````markdown", readLines("examples/example_tufte.qmd"), "`````", sep = "\n")
```
