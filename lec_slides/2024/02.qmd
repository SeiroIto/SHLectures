...
治療群と統御群が似ていても、推計結果の信頼性が低い場合、得られた知識が歪んでいる場合があります
...
* 似た結果指標(例: 疲労と労働時間)への効果推計&larr;multiple testingを無意識にやっている<gray>similar outcome indicators (e.g., fatigue and work hours)&larr;unintentional multiple testing</gray>
   * 対策: 同じ結果指標グループの$p$値平均値family wise error rateを使う、同じ結果指標グループを指数化する<gray>Countermeasures: use the mean of $p$ values of similar outcomes (called "family wise error rate"), create a single index of similar outcomes using the normalised estimates and their standard errors</gray>
      ...
      * 指数化=効果を(結果指標の標準偏差で割ることで)標準偏差単位にして(標準化standardisationという)、結果指標1を指標1の標準偏差単位でx1単位高める、結果指標1を指標1の標準偏差単位でx1単位高める、と足し算できるようにし、標準化された効果の平均値とその標準誤差を計算し、$p$ valueを得る
