...
* 最初の3行: パッケージをインストールし、読み込みます
* 4-5: DOSパッケージのangristlavyデータを読み込み、data.table形式にしてaldataというオブジェクトにする
* 6: aldataを使って、avgmath = a + $b_{1}$clasz + $b_{2}$tipuachを推計、推計結果をlm1というオブジェクトにする
* 7: aldataを使って、avgmath = a + $c_{1}$tipuachを推計、推計結果をlm11というオブジェクトにする
* 8: aldataを使って、clasz = a + $d_{1}$tipuachを推計、推計結果をlm12というオブジェクトにする
* 9: lm11の残差、lm12の残差を2列のデータにし、rdataというオブジェクトにする
* 10: rdataを使って、avgmath = A + $B_{1}$claszを推計、推計結果をlm13というオブジェクトにする
* 11: 行頭に#の付いたコメント行なのでRに無視される
:::
:::
