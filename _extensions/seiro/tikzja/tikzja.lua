...
local globalOptions = {
  ...
  cache = nil,
  jfont = "Meiryo",   -- Japanese font; any installed system font name
  border = "4pt"      -- standalone crop margin
}
...
local function createTexFile(tikzCode, tmpdir, outputFile, scale, libraries, jfont, border)
  ...
  jfont = jfont or "Meiryo"
  border = border or "4pt"

  local template = [[
\documentclass[tikz,border=%s]{standalone}
\usepackage{luatexja}
...
\end{document}
  ...
  local texCode = string.format(template, border, jfont, libraryNames, scale, tikzCode)
...
local function tikzToSvg(tikzCode, tmpdir, outputFile, scale, libraries, jfont, border)
  local texFile = createTexFile(tikzCode, tmpdir, outputFile, scale, libraries, jfont, border)
  local pdfFile = pandoc.path.join({ tmpdir, outputFile .. ".pdf" })
  local svgFile = pandoc.path.join({ tmpdir, outputFile .. ".svg" })

  -- NOTE (fork): LuaTeX-ja refuses DVI output ("DVI output is not supported in
  -- LuaTeX-ja"), so we go lualatex -> PDF (its supported mode) -> pdftocairo -> SVG.
  -- dvisvgm's own --pdf mode is not usable here: it needs Ghostscript < 10.01 or mutool.
  local _, _, latexExitCode = os.execute("lualatex -interaction=nonstopmode -output-directory=" .. tmpdir .. " " .. texFile)
  -- lualatex returns 1 on mere warnings (luatexja/fontspec are chatty) even when the
  -- PDF was written. Judge by the artifact, not the exit code.
  if not file_exists(pdfFile) then
    error("lualatex produced no PDF (exit code " .. tostring(latexExitCode) .. ")")
  end

  local _, _, cairoExitCode = os.execute("pdftocairo -svg " .. pdfFile .. " " .. svgFile)
  if not file_exists(svgFile) then
    error("pdftocairo produced no SVG (exit code " .. tostring(cairoExitCode) .. ")")
  end
...
local function tikzToPdf(tikzCode, tmpdir, outputFile, scale, libraries, jfont, border)
  local texFile = createTexFile(tikzCode, tmpdir, outputFile, scale, libraries, jfont, border)
  local pdfFile = pandoc.path.join({ tmpdir, outputFile .. ".pdf" })
...
local function renderTikz(cb, options, tmpdir)
  ...
  -- NOTE (fork): upstream wrapped the whole generation block in `if options.cache then`,
  -- so with the default cache=nil nothing was ever generated and the read below failed.
  -- Generation now always runs; the cache is only consulted when explicitly enabled.
  local cachePath
  local served_from_cache = false
